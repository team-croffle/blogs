...
<template>
  <div v-if="visible" class="group relative inline-flex items-center">
    <span class="text-fg-50 inline-flex items-center gap-1" :aria-label="`라이선스 ${licenseCode}`">
    ...
    <div
      role="tooltip"
      class="glass-palette text-foreground pointer-events-none absolute top-full left-0 z-20 mt-2 w-max max-w-80 rounded-[12px] px-3 py-2 text-left text-xs leading-relaxed opacity-0 shadow-md transition-opacity duration-150 group-focus-within:opacity-100 group-hover:opacity-100"
    >
